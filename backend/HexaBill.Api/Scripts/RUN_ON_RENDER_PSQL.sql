...
-- =============================================================================
-- PaymentReceipts / RecurringInvoices: Id identity (fixes receipt 500 / 23502)
-- and RecurringInvoices.IsActive integer→boolean (fixes daily recurring job)
-- =============================================================================
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'PaymentReceipts' AND column_name = 'Id'
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'RecurringInvoices' AND column_name = 'Id'
          AND is_identity = 'NO' AND column_default IS NULL
    ) THEN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'RecurringInvoiceItems' AND column_name = 'Id'
          AND is_identity = 'NO' AND column_default IS NULL
    ) THEN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'RecurringInvoices' AND column_name = 'IsActive'
        ...
        ALTER TABLE "RecurringInvoices"
            ...
            USING (CASE WHEN "IsActive"::int = 0 THEN false ELSE true END);
...
-- =============================================================================
-- CustomerItemPrices — last billed exclusive unit price per customer+product
-- Returns/voids do NOT roll back this memory (application rule).
-- =============================================================================
CREATE TABLE IF NOT EXISTS "CustomerItemPrices" (
    ...
    "LastUnitPrice" numeric(18,2) NOT NULL,
    "LastSaleId" integer NULL,
    "LastSaleDate" timestamp with time zone NOT NULL,
    "UpdatedAt" timestamp with time zone NOT NULL,
    CONSTRAINT "FK_CustomerItemPrices_Customers_CustomerId"
        FOREIGN KEY ("CustomerId") REFERENCES "Customers" ("Id") ON DELETE CASCADE,
    CONSTRAINT "FK_CustomerItemPrices_Products_ProductId"
        FOREIGN KEY ("ProductId") REFERENCES "Products" ("Id") ON DELETE RESTRICT,
    CONSTRAINT "FK_CustomerItemPrices_Sales_LastSaleId"
        FOREIGN KEY ("LastSaleId") REFERENCES "Sales" ("Id") ON DELETE SET NULL
);

CREATE UNIQUE INDEX IF NOT EXISTS "IX_CustomerItemPrices_TenantId_CustomerId_ProductId"
    ON "CustomerItemPrices" ("TenantId", "CustomerId", "ProductId");
CREATE INDEX IF NOT EXISTS "IX_CustomerItemPrices_CustomerId" ON "CustomerItemPrices" ("CustomerId");
CREATE INDEX IF NOT EXISTS "IX_CustomerItemPrices_ProductId" ON "CustomerItemPrices" ("ProductId");
CREATE INDEX IF NOT EXISTS "IX_CustomerItemPrices_LastSaleId" ON "CustomerItemPrices" ("LastSaleId");

-- =============================================================================
-- Feature 4: Customer stop location (main pin on Customer; visit GPS audit)
-- =============================================================================
ALTER TABLE "Customers" ADD COLUMN IF NOT EXISTS "MainLatitude" numeric(9,6) NULL;
ALTER TABLE "Customers" ADD COLUMN IF NOT EXISTS "MainLongitude" numeric(9,6) NULL;
ALTER TABLE "Customers" ADD COLUMN IF NOT EXISTS "LocationUpdatedAt" timestamp with time zone NULL;
ALTER TABLE "Customers" ADD COLUMN IF NOT EXISTS "LocationUpdatedBy" integer NULL;

ALTER TABLE "CustomerVisits" ADD COLUMN IF NOT EXISTS "Latitude" numeric(9,6) NULL;
ALTER TABLE "CustomerVisits" ADD COLUMN IF NOT EXISTS "Longitude" numeric(9,6) NULL;
ALTER TABLE "CustomerVisits" ADD COLUMN IF NOT EXISTS "ReachedAt" timestamp with time zone NULL;

ALTER TABLE "RouteCustomers" ADD COLUMN IF NOT EXISTS "SortOrder" integer NULL;
