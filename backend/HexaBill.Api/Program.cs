...
builder.Services.AddScoped<ICustomerItemPriceService, CustomerItemPriceService>();
    ...
    try
    {
        using (var scopeSlc = app.Services.CreateScope())
        {
            ...
            if (ctxSlc.Database.IsNpgsql())
            {
                ...
                // Payment receipts / recurring invoices: SQLite-oriented migrations created Id without
                // PostgreSQL identity, so EF inserts NULL Id → 23502 and client "Receipt could not be loaded".
                // Also fix RecurringInvoices.IsActive integer→boolean (model is bool; job fails at 00:05).
                ctxSlc.Database.ExecuteSqlRaw(@"
                    DO $$
                    BEGIN
                        IF EXISTS (
                        ...
                        END IF;
                    END $$;");
                startupLog?.LogInformation("PaymentReceipts/RecurringInvoices Id identity + IsActive bool ensured at startup.");
            }
        }
    }
    catch (Exception ex)
