    ...
    <div className="h-full min-h-0 flex flex-col overflow-hidden w-full max-w-full p-2 md:p-3">
      <div className="shrink-0 flex items-center justify-between mb-2 gap-2">
        <div className="min-w-0">
          <h1 className="text-lg md:text-xl font-bold text-text-primary leading-tight">Agreements</h1>
          <p className="text-xs text-text-secondary truncate">Create, edit, reprint, delete</p>
        </div>
        <Link
          to="/agreements/new"
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          <ScrollText className="w-9 h-9 mx-auto mb-2 text-slate-400" />
          <p className="text-text-primary font-medium mb-1">No agreements yet</p>
          <p className="text-sm text-text-secondary mb-3">Create your first agreement — fill Second Party details, then print PDF.</p>
          <Link
            to="/agreements/new"
          ...
          >
            <Plus className="w-4 h-4" /> Create first agreement
          </Link>
        </div>
      ) : (
          ...
          <table className="w-full text-sm min-w-[720px]">
            <thead className="bg-slate-50 text-left sticky top-0 z-10">
              <tr>
                ...
                <th className="px-2 py-1.5 font-semibold">Second Party</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t hover:bg-slate-50">
                  ...
                  <td className="px-2 py-1">{String(r.agreementDate || '').slice(0, 10)}</td>
                  <td className="px-2 py-1">{r.secondPartyName || '—'}</td>
