...
const PosPage = () => {
  ...
  const openPickerAfterAddRef = useRef(false)
  const focusedCartRowRef = useRef(null)
  const handleSaveRef = useRef(null)
  const handleHoldRef = useRef(null)
  const handleNewInvoiceRef = useRef(null)
  const addEmptyRowRef = useRef(null)
  const openProductPickerRef = useRef(null)
  ...
  /** Paginate picker flat list (10 per page). Highlight is page-local (0..pageItems.length-1). */
  ...
  const focusProductPickerSearch = () => {
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        const candidates = [productPickerSearchDesktopRef.current, productPickerSearchMobileRef.current]
  ...
  const openProductPicker = (rowIndex) => {
    ...
    focusedCartRowRef.current = rowIndex
    focusProductPickerSearch()
  }
  ...
  const addEmptyRow = () => {
    ...
    openPickerAfterAddRef.current = true
    focusedCartRowRef.current = newIndex
    setCart([...cart, {
      productId: null,
      productName: '',
      sku: '',
  ...
  // After Add Row: open product drawer + focus search (Tally cashier flow)
  useEffect(() => {
    if (!openPickerAfterAddRef.current) return
    const idx = lastAddedRowIndexRef.current
    if (idx === null) return
    openPickerAfterAddRef.current = false
  ...
  const focusCartField = (rowIndex, field) => {
    if (field === 'product' && productPickerRowIndex === rowIndex) {
      focusProductPickerSearch()
      return
    }
    const refMap = {
  ...
  const handleProductSearchKeyDown = (e, rowIndex) => {
    ...
    const { flat, pageItems, page, totalPages } = getPickerPagination(rowIndex)
    ...
    if (dropdownOpen && pageItems.length > 0 && e.key === 'ArrowDown') {
      e.preventDefault()
      if (hi < pageItems.length - 1) {
        setProductHighlightByRow(prev => ({ ...prev, [rowIndex]: hi + 1 }))
      } else if (page < totalPages - 1) {
        setProductPickerPage(page + 1)
        setProductHighlightByRow(prev => ({ ...prev, [rowIndex]: 0 }))
      }
      return
    }
    if (dropdownOpen && pageItems.length > 0 && e.key === 'ArrowUp') {
      e.preventDefault()
      if (hi > 0) {
        setProductHighlightByRow(prev => ({ ...prev, [rowIndex]: hi - 1 }))
      } else if (page > 0) {
        const prevLen = Math.min(PRODUCT_PICKER_PAGE_SIZE, flat.length - (page - 1) * PRODUCT_PICKER_PAGE_SIZE)
        setProductPickerPage(page - 1)
        setProductHighlightByRow(prev => ({ ...prev, [rowIndex]: Math.max(0, prevLen - 1) }))
      }
      return
    }

    if (e.key === 'Enter' || (e.key === 'Tab' && !e.shiftKey && dropdownOpen && pageItems.length > 0)) {
      if (dropdownOpen && pageItems.length > 0) {
        e.preventDefault()
        const product = pageItems[Math.min(hi, pageItems.length - 1)] || pageItems[0]
  ...
  addEmptyRowRef.current = addEmptyRow
  openProductPickerRef.current = openProductPicker
  ...
  useEffect(() => {
    ...
    const openPickerOnActiveRow = () => {
      if (loading || loadingSale) return
      const emptyIdx = cart.findIndex(item => !item.productId)
      if (emptyIdx >= 0) {
        openProductPickerRef.current?.(emptyIdx)
        return
      }
      const focused = focusedCartRowRef.current
      if (focused != null && focused >= 0 && focused < cart.length) {
        openProductPickerRef.current?.(focused)
        return
      }
      addEmptyRowRef.current?.()
    }
    ...
    const onKey = (e) => {
      ...
      if (e.key === 'F3' || ((e.key === 'l' || e.key === 'L') && (e.ctrlKey || e.metaKey))) {
        e.preventDefault()
        openPickerOnActiveRow()
        return
      }
      if (e.key === 'F4') {
        e.preventDefault()
      ...
      if (e.key === 'F8') {
        e.preventDefault()
        if (!isZeroInvoice) setShowDiscountPopup(true)
        return
      }
      if (e.key === 'F10') {
        e.preventDefault()
        handleNewInvoiceRef.current?.()
        return
      }
      if (e.key === 'Delete' && !isTypingTarget(e.target)) {
        e.preventDefault()
        const idx = productPickerRowIndex ?? focusedCartRowRef.current
        if (idx != null && idx >= 0 && idx < cart.length) {
          removeFromCartRef.current?.(idx)
        }
      }
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [loading, loadingSale, cart, productPickerRowIndex, isZeroInvoice])
  ...
  return (
    <div className="h-[100dvh] max-h-[100dvh] bg-[#F8FAFC] flex flex-col max-w-full overflow-hidden pb-24 lg:pb-0">
      {/* Compact cashier toolbar (~48–60px) */}
      <div className="bg-primary-900 text-white px-2 sm:px-3 h-12 sm:h-14 flex items-center justify-between gap-2 flex-shrink-0">
      ...
      {/* Compact meta row (~40–56px) */}
      <div className="bg-white border-b border-[#E5E7EB] px-2 sm:px-3 py-1.5 flex-shrink-0">
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 min-h-10">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden w-full">
        {/* Items Table + product picker */}
        <div className="flex-1 overflow-hidden bg-neutral-50 min-w-0 min-h-0" style={{ display: 'flex', flexDirection: 'column' }}>
          <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden">
          <div className="flex-1 min-w-0 overflow-y-auto px-2 py-1.5 md:pb-1">
            {/* Desktop Table View */}
            <div className="hidden md:block bg-white rounded-lg border border-gray-300 shadow-sm overflow-x-auto max-h-full">
              <div>
                <table className="w-full text-xs sm:text-sm border-collapse" style={{ tableLayout: 'auto' }}>
                  <thead className="bg-gray-100 border-b border-gray-300 sticky top-0 z-10">
                    <tr>
                      <th className="px-1 sm:px-1.5 py-1 text-left font-bold text-gray-900 border-r border-gray-300 whitespace-nowrap w-10 text-xs">SL<br /><span className="text-[10px] font-normal text-gray-600">رقم</span></th>
                      <th className="px-1 sm:px-1.5 py-1 text-left font-bold text-gray-900 border-r border-gray-300 text-xs w-[28%] max-w-xs">Description<br /><span className="text-[10px] font-normal text-gray-600">التفاصيل</span></th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {cart.length === 0 ? (
                      ...
                      cart.map((item, index) => (
                        <tr
                          key={index}
                          className={`border-b border-gray-200 even:bg-neutral-50 hover:bg-gray-50 ${productPickerRowIndex === index ? 'bg-primary-50 ring-1 ring-inset ring-primary-300' : ''}`}
                          onClick={() => {
                          ...
                          onFocusCapture={() => { focusedCartRowRef.current = index }}
                        >
                          <td className="px-1 sm:px-1.5 py-0.5 text-center border-r border-gray-200 font-medium text-xs align-middle">{index + 1}</td>
                          <td className="px-1 sm:px-1.5 py-0.5 border-r border-gray-200 align-middle w-[28%] max-w-xs">
          ...
          {/* Desktop product picker — PO Discount drawer chrome */}
          {productPickerRowIndex !== null && (
            <aside
              className="product-picker-panel hidden md:flex w-[28rem] xl:w-[30rem] shrink-0 flex-col border border-neutral-200 bg-white shadow-xl min-h-0 rounded-l-lg"
            ...
            >
              <div className="flex items-start justify-between px-4 py-3 border-b border-neutral-200 shrink-0">
                <div className="min-w-0 pr-2">
                  <p className="text-base font-bold text-neutral-900">Products</p>
                  <p className="text-xs text-neutral-500 mt-0.5">Search and select for row {productPickerRowIndex + 1}</p>
                </div>
                <button
                  type="button"
              ...
              <div className="shrink-0 px-4 py-2 border-b border-neutral-100 bg-neutral-50">
                <input
                  ref={productPickerSearchDesktopRef}
                  type="search"
                  value={productSearchTerms[productPickerRowIndex] || ''}
                  disabled={isFormDisabled}
                  onChange={(e) => {
                    const searchValue = e.target.value
                    setProductSearchTerms(prev => ({ ...prev, [productPickerRowIndex]: searchValue }))
                    setProductHighlightByRow(prev => ({ ...prev, [productPickerRowIndex]: 0 }))
                    setProductPickerPage(0)
                  }}
                  onKeyDown={(e) => handleProductSearchKeyDown(e, productPickerRowIndex)}
                  placeholder="Search name, SKU, barcode…"
                  className="w-full h-11 min-h-[44px] px-3 border border-neutral-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:opacity-50"
                  aria-label="Search products"
                />
                <p className="text-[10px] text-neutral-500 mt-1">↑↓ Enter Esc · 10 per page · Recent + catalog on open</p>
              </div>
              <div className="flex-1 overflow-y-auto min-h-0">
                {loadingProducts ? (
                  <div className="p-4 text-center text-sm text-neutral-500">Loading products...</div>
                ) : (() => {
                  const { recent, flat, pageItems, page, totalPages, start } = getPickerPagination(productPickerRowIndex)
                  const searchTerm = productSearchTerms[productPickerRowIndex] || ''
                  const hi = productHighlightByRow[productPickerRowIndex] ?? 0
                  const recentIds = new Set(recent.map(p => String(p.id)))
                  if (flat.length === 0) {
                  ...
                  return (
                    <>
                      {pageItems.map((product, localIdx) => {
                        const isRecent = !searchTerm.trim() && recentIds.has(String(product.id))
                        const headers = []
                        if (!searchTerm.trim() && page === 0 && isRecent && !showedRecentHeader) {
                          showedRecentHeader = true
                          headers.push(
                            <div key="hdr-recent" className="px-4 py-1 bg-slate-100 border-b border-slate-200 text-[10px] font-semibold uppercase tracking-wide text-slate-600">
                              Recent
                            </div>
                          )
                        }
                        if (!searchTerm.trim() && !isRecent && !showedAllHeader) {
                          showedAllHeader = true
                          headers.push(
                            <div key="hdr-all" className="px-4 py-1 bg-slate-50 border-b border-slate-200 text-[10px] font-semibold uppercase tracking-wide text-slate-600">
                              All products
                            </div>
                          )
                        }
                        return (
                          <div key={`${product.id}-${start + localIdx}`}>
                            ...
                            <button
                              type="button"
                              className={`w-full text-left px-4 py-1.5 border-b border-neutral-100 hover:bg-slate-50 grid grid-cols-[1fr_auto] gap-x-2 gap-y-0.5 items-center ${hi === localIdx ? 'bg-primary-50 ring-1 ring-inset ring-primary-300' : ''}`}
                              onMouseEnter={() => setProductHighlightByRow(prev => ({ ...prev, [productPickerRowIndex]: localIdx }))}
                            ...
                            >
                              ...
                              <span className="text-xs text-neutral-600 tabular-nums">AED {(product.sellPrice ?? 0).toFixed(2)}</span>
                              <span className={`text-xs font-semibold text-right tabular-nums ${product.stockQty <= (product.reorderLevel || 0) ? 'text-red-600' : 'text-green-600'}`}>
                                {product.stockQty}
                              </span>
                            </button>
                          </div>
              ...
              {(() => {
                const { flat, page, totalPages } = getPickerPagination(productPickerRowIndex)
                ...
                return (
                  <div className="shrink-0 flex items-center justify-between gap-2 px-3 py-2 border-t border-neutral-200 bg-neutral-50">
                    <button
                      ...
                      disabled={page <= 0}
                      onClick={() => {
                        setProductPickerPage(p => Math.max(0, p - 1))
                        setProductHighlightByRow(prev => ({ ...prev, [productPickerRowIndex]: 0 }))
                      }}
                      className="inline-flex items-center gap-0.5 px-2 py-1.5 text-xs font-medium border border-neutral-300 rounded-lg hover:bg-white disabled:opacity-40"
                    >
                      <ChevronLeft className="h-3.5 w-3.5" /> Back
                    </button>
                    <span className="text-xs text-neutral-600 tabular-nums">Page {page + 1} of {totalPages}</span>
                    <button
                      type="button"
          ...
          {/* Mobile product picker bottom sheet — same pagination + PO chrome */}
          {productPickerRowIndex !== null && (
            <div className="md:hidden fixed inset-0 z-[10000]" data-product-picker="mobile" role="dialog" aria-modal="true" aria-label="Product picker">
              ...
              <div className="absolute bottom-0 left-0 right-0 max-h-[70vh] bg-white rounded-t-2xl shadow-xl flex flex-col border-t border-neutral-200">
                <div className="flex items-start justify-between px-4 py-3 border-b border-neutral-200 shrink-0">
                  <div>
                    <p className="text-base font-bold text-neutral-900">Products</p>
                    <p className="text-xs text-neutral-500">Search and select for row {productPickerRowIndex + 1}</p>
                  </div>
                  <button type="button" onClick={() => closeProductPicker(productPickerRowIndex)} className="p-1.5 min-h-[44px] min-w-[44px]" aria-label="Close">
                    <X className="h-5 w-5" />
                  </button>
                </div>
                <div className="shrink-0 px-4 py-2 border-b border-neutral-100">
                  <input
                    ref={productPickerSearchMobileRef}
                    type="search"
                    value={productSearchTerms[productPickerRowIndex] || ''}
                    disabled={isFormDisabled}
                    onChange={(e) => {
                ...
                <div className="overflow-y-auto flex-1 min-h-0">
                  {loadingProducts ? (
                    ...
                    const { flat, pageItems, page } = getPickerPagination(productPickerRowIndex)
                    const hi = productHighlightByRow[productPickerRowIndex] ?? 0
                    if (flat.length === 0) {
                      return <div className="p-4 text-center text-sm text-neutral-500">No products found</div>
                    }
                    return pageItems.map((product, localIdx) => (
                      <button
                        key={`${product.id}-m-${page}-${localIdx}`}
                        type="button"
                        className={`w-full text-left px-4 py-2.5 border-b border-neutral-100 min-h-[44px] ${hi === localIdx ? 'bg-primary-50' : ''}`}
                        onClick={() => selectProduct(product, productPickerRowIndex)}
                      >
                ...
                {(() => {
                  ...
                  if (flat.length === 0) return null
                  return (
                    <div className="shrink-0 flex items-center justify-between gap-2 px-3 py-2 border-t border-neutral-200 bg-neutral-50 pb-safe">
                      <button
                        type="button"
                        disabled={page <= 0}
                        onClick={() => {
                      ...
                      <span className="text-xs text-neutral-600">Page {page + 1} of {totalPages}</span>
                      <button
                        type="button"
                        disabled={page >= totalPages - 1}
                        onClick={() => {
        ...
        {/* Sticky footer: totals + collapsed payment + Save */}
        <div className="bg-white border-t border-[#E5E7EB] p-2 sm:p-2.5 shrink-0 sticky bottom-0 z-20 shadow-[0_-2px_8px_rgba(0,0,0,0.04)]">
